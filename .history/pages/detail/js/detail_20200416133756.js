(function (window, $, doc) {
  ...
  Star.prototype = {
            ...
            $.each(jzxSumData, (index, ele) => {
              str1 =
                ...
                `<li class="right__infoItem">
                 <span class="right__itemTitle">${ele.title1}</span>
                 <span class="right__itemCon">${ele.value1}</span>
                 <span class="right__itemTitle">${ele.title2}</span>
                 <span class="right__itemCon">${ele.value2}</span>
                 <span class="right__itemTitle">${ele.title3}</span>
                 <span class="right__itemCon">${ele.value3}</span>
                 <span class="right__itemTitle">${ele.title4}</span>
                 <span class="right__itemCon">${ele.value4}</span>
                 <span class="right__itemTitle">${ele.title5}</span>
                 <span class="right__itemCon">${ele.value5}</span>
                 <span class="right__itemTitle">${ele.title6}</span>
                 <span class="right__itemCon">${ele.value6}</span>
                 <span class="right__itemTitle">${ele.title7}</span>
                 <span class="right__itemCon">${ele.value7}</span>
                 </li>`;
            });
    ...
    createEcha__jxz(sanX, sanY, zexianX, zexianY) {
      ...
      setInterval(() => {
        let sanY = [
          _this.getRange(5000, 30000),
          _this.getRange(5000, 30000),
          _this.getRange(5000, 30000),
          _this.getRange(5000, 30000),
          _this.getRange(5000, 30000),
        ];
        opt2.series[0].data = sanY;
        myChart2.setOption(opt2, true);
      }, 5000);
    },
    createViewByCl() {
          ...
          .then((res) => {
            ...
            $.each(clInfoData, (index, ele) => {
              str1 =
                ...
                <span class="right__itemw jkcl">${ele.value}</span>
                </li>`;
            });
    ...
    updateCl() {
      ...
      $.each(jkcl,(index,ele)=>{
          let num = this.getRange(0,10)

          $(ele).text( _this.randomCl(num))
    ...
    randomCl(data) {
      ...
      let index = Math.floor(Math.random()*(0 - 50) + 50);
      if(!index) index = 1 console.log(index) ;

      if (data < 5) {
        return `[外集卡] ${arr1[index]}${String.fromCharCode( this.getRange(65,95))}${this.getRange(0,9)}${this.getRange(0,9)}${this.getRange(0,9)}${this.getRange(0,9)}`

      } else {

         return `[内集卡] 厦门港HB·${this.getRange(0,9)}${this.getRange(0,9)}${this.getRange(0,9)}`
    ...
    createEcha__cl(zexianX, zexianY) {
      let _this = this;
      let myChart2 = echarts.init(document.getElementById("right__zexianC"));
      let opt = {
        grid: {
          x: "16%", //x 偏移量
          y: "7%", // y 偏移量
      ...
      setInterval(() => {
        let sanY = [
          _this.getRange(40, 70),
          _this.getRange(40, 70),
          _this.getRange(40, 70),
          _this.getRange(40, 70),
          _this.getRange(40, 70),
        ];
        opt.series[0].data = sanY;
            ...
            $.each(dzwlInfoDataSum3, (index, ele) => {
              str3 =
                ...
                <div class="con__sumP">
                  ...
                  <span class="con__sumPc"> <span class="con__sumPcItem" style="width:${Math.floor(
                    (ele.valOne / 150) * 100
                  )}%"></span></span>
    ...
    createViewWlzb() {
      if (this.form === "106" && this.type === "total") {
        this.getData(window.baseUrl.wlznNumUrl)
          .then((res) => {
            let { wlzbNum, wlzbNumMonth, wlzbTrendX, wlzbTrendY } = res;

            let STR = `<div class="right__num2" id="right__num2"></div>
          <div class="con__ybp">
          <span class="con__subT">网络指标成功率统计</span>
          <div class="con__yibiao">
            <div class="con__yibiaoItem">
              <div class="con__ybCon" id="con__ybCon1"></div>
              <span class="conybW">RRC建立成功率</span>
            </div>
            <div class="con__yibiaoItem">
              <div class="con__ybCon" id="con__ybCon2"></div>
              <span class="conybW">NG信令连接成功率</span>
            </div>
            <div class="con__yibiaoItem">
              <div class="con__ybCon" id="con__ybCon3"></div>
              <span class="conybW">QosFlow建立成功率</span>
            </div>
            <div class="con__yibiaoItem">
              <div class="con__ybCon" id="con__ybCon4"></div>
              <span class="conybW">呼叫建立成功率</span>
            </div>
            <div class="con__yibiaoItem">
              <div class="con__ybCon" id="con__ybCon5"></div>
              <span class="conybW">QosFlow掉活率</span>
            </div>
            <div class="con__yibiaoItem">
              <div class="con__ybCon" id="con__ybCon6"></div>
              <span class="conybW">RRC连接重建比</span>
            </div>
          </div>
        </div>
          <div class="right__zexian" id="right__zexian1"></div>
        ...
        this.getData(window.baseUrl.wlznSumNumUrl)
          .then((res) => {
           ...
           <span class="right__wlzbW">
              ...
              <span class="right__wlzbWR user" id="user">${ele.valueone}</span>
           </span>
           <span class="right__wlzbW">
             <span class="right__wlzbWL">当天实时上下业务量累计数量:</span>
              <span class="right__wlzbWR yewu" id="yewu">${ele.valueTwo}</span>
           </span>
           <span class="right__wlzbW">
             <span class="right__wlzbWL">上行PRB实时利用率:</span>
              <span class="right__wlzbWR PRB1" id="PRB1">${ele.valueThr}</span>
           </span>
           <span class="right__wlzbW">
             <span class="right__wlzbWL">下行PRB实时利用率:</span>
              <span class="right__wlzbWR PRB2" id="PRB2">${ele.valuefour}</span>
           </span> 
        </li>`;
            });
    ...
    /**
     * @Author fyt
     * @Description 更新网络概述数据
     * @Date 2020-04-16 12:26:24 星期四
     */
    updataYibiao() {
      let _this = this;
      let uesr = $(".user");
      ...
      setInterval(() => {
        $.each(uesr, (index, ele) => {
          $(ele).text(`${_this.getRange(30, 60)}个`);
        });
        $.each(yewu, (index, ele) => {
          $(ele).text(`${_this.getRange(1, 10)}GB`);
        });
        $.each(PRB1, (index, ele) => {
          $(ele).text(`${_this.getRange(20.1, 30.9)}%`);
        });
        $.each(PRB2, (index, ele) => {
          $(ele).text(`${_this.getRange(10.1, 19.9)}%`);
        });
      }, 5000);
    },
    /**
    ...
    handleTotal() {
      this.type = "total";
      document.getElementById("total__img").src =
        "../../../css/img/zongliang.png";
